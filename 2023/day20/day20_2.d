...
{
    long[string] interesting_mods = ["qb" : long.max, "nn" : long.max, "rd" : long.max, "rl" : long.max];
    ...
    foreach (T; 0..1000_000)
    {
        ...
        while (!queue.empty)
        {
            ...
            else if (type == '&')
            {
                ...
                if (pulse == 0 && (sig[0] in interesting_mods))
                {
                    //writeln(sig[0], " ", T);
                    interesting_mods[sig[0]] = min(interesting_mods[sig[0]], T+1);

                    if (interesting_mods.values.all!(x => x != long.max))
                    {
                        writeln(interesting_mods.values.fold!((a, b) => lcm(a, b))(1L));
                        // 4093
                        // 4091
                        // 3911
                        // 3733

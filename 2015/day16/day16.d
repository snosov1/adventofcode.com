...
int dyn(Aunt aunt, string field)
{
    foreach (m; __traits(allMembers, Aunt))
    {
        if (m == field)
        {
            return mixin("aunt." ~ m);
...
int main(string[] args)
{
    ...
    foreach (line; stdin.byLine)
    {
        ...
        if (line
                ...
                .all!((a){
                    auto q = a.findSplit(": ");
                    auto what = q[0][1..$].to!string;
                    auto howmany = q[2].to!int;
                    if (what == "cats" || what == "trees")
                        return theone.dyn(what) < howmany;
                    else if (what == "pomeranians" || what == "goldfish")
                        return theone.dyn(what) > howmany;
                    else
                        return theone.dyn(what) == howmany;
                })
